...
export default function NewArticlePage({articleId ,placeholder=""}) {
  ...
  //               setPartition(art?.partition || "SQUARE");
  //               setDescription(art?.description || "");
  //               setIsPublished(art?.status);
  ...
  const handleSubmit = async (e) => {
    ...
    const coverImageId = (imageUrls[coverIndex] || imageUrls[0] || "").replace(
      /^\/api\/images\//,
      ""
    );
  ...
  const handleFilesChange = async (e) => {
    ...
    try {
      ...
      if (
        ...
        Array.isArray(uploadResult.ids)
      ) {
        // 组装文件信息，自动带上id
  ...
  return (
    <div
      className="fixed inset-0 w-full min-h-screen bg-gradient-to-br from-indigo-50 via-white to-blue-50 flex flex-col"
    ...
    >
      ...
      />
      <div
        className="flex-1 flex flex-row items-stretch px-0 pb-8"
        style={{ gap: "48px", width: "100vw" }}
      >
        {/* 左侧：文章配置卡片 3/10 */}
        <div
          style={{
        ...
        >
          ...
          {showLeft && (
            ...
            >
              <h1 className="text-2xl font-extrabold mb-8 text-indigo-700 text-center tracking-tight drop-shadow">
                文章配置
              </h1>
              <form
              ...
              >
                <div>
                  <label
                    htmlFor="title"
                    className="block text-base font-semibold mb-1 text-indigo-900"
                  >
                    标题
                  </label>
                  <input
                ...
                <div className="flex items-center">
                  ...
                  <label
                    htmlFor="isPublished"
                    className="ml-3 block text-base text-indigo-900"
                  >
                    公开
                  </label>
                </div>
        ...
        >
          <h2 className="text-2xl font-bold mb-6 text-indigo-700 text-center drop-shadow">
            文章内容
          </h2>
        ...
        >
          ...
          {showRight && (
            <div
              ...
              style={{ color: "#222", boxShadow: "0 8px 32px #6366f122" }}
            >
              <h2 className="text-xl font-bold mb-6 text-indigo-700 text-center drop-shadow">
                图片
              </h2>
              {/* 图片区 */}
                ...
                {imageFiles.length === 0 ? (
                  ...
                  >
                    ...
                    >
                      ...
                      />
                      <path
                        d="M3 17l4.5-6a2 2 0 0 1 3.2-.2l2.6 3.2a2 2 0 0 0 3.2-.2L21 9"
                        stroke="#a5b4fc"
                        strokeWidth="1.5"
                        strokeLinecap="round"
                      />
                    </svg>
                    <div className="mt-2 text-base group-hover:text-indigo-500 transition">
                      还未添加图片，点击此处添加
                    </div>
                    <input
                    ...
                    <div className="flex flex-col gap-2 mt-2 w-full">
                      ...
                      <div className="flex flex-wrap gap-3">
                        {imageFiles.map((file, idx) => (
                          <div
                            key={idx}
                            className={`relative flex flex-col items-center border-2 rounded-xl p-2 transition cursor-pointer group ${
                              coverIndex === idx
                                ? "border-indigo-500 bg-indigo-50/80 shadow-lg scale-105"
                                : "border-gray-200 bg-white/80 hover:scale-105 hover:shadow-md"
                            }`}
                            style={{
                              width: 100,
                              boxShadow:
                                coverIndex === idx ? "0 0 0 3px #6366f1aa" : "",
                            }}
                            onClick={() =>
                              insertImageMarkdown(file, imageUrls[idx] || "")
                            }
                          >
                            ...
                            <span
                              className="text-xs truncate max-w-[80px] text-gray-700 mb-1"
                              title={file.name}
                            >
                              {file.name}
                            </span>
                            <label
                              className="flex items-center gap-1 cursor-pointer select-none mt-1"
              ...
              {/* 渐变横线分隔 */}
              <div
                className="w-full h-px my-4 bg-gradient-to-r from-transparent via-indigo-200 to-transparent"
                style={{ opacity: 0.8 }}
              ></div>
              {/* 文件区 */}
              <div className="flex flex-col gap-2 w-full flex-1 overflow-auto">
                <h2 className="text-xl font-bold mb-4 text-indigo-700 text-center drop-shadow">
                  文件
                </h2>
                    ...
                    {fileList.map((file, idx) => (
                      ...
                      >
                        ...
                        <a
                          ...
                          className="text-indigo-500 hover:underline text-xs px-2"
                        >
                          下载
                        </a>
                        <button
