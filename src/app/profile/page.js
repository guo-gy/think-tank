        ...
        <div className="grid grid-cols-1 md:grid-cols-10 gap-8 w-full h-full">
          ...
          <aside className="md:col-span-3 col-span-1 bg-white rounded-2xl shadow-lg border border-gray-100 p-6 flex flex-col h-full">
            ...
            <ul className="space-y-2">
              <li>
                <button className={`w-full text-left px-4 py-2 rounded-lg transition-colors duration-200 ${tab === 'my-articles' ? 'bg-indigo-100 text-indigo-700 font-semibold' : 'hover:bg-gray-50 text-gray-700'}`} onClick={() => setTab('my-articles')}>自己写的文章</button>
              </li>
              <li>
                <button className={`w-full text-left px-4 py-2 rounded-lg transition-colors duration-200 ${tab === 'liked-articles' ? 'bg-indigo-100 text-indigo-700 font-semibold' : 'hover:bg-gray-50 text-gray-700'}`} onClick={() => setTab('liked-articles')}>点赞过的文章</button>
              </li>
              <li>
                <button className={`w-full text-left px-4 py-2 rounded-lg transition-colors duration-200 ${tab === 'change-password' ? 'bg-indigo-100 text-indigo-700 font-semibold' : 'hover:bg-gray-50 text-gray-700'}`} onClick={() => setTab('change-password')}>修改密码</button>
              </li>
            </ul>
          </aside>
          {/* 右侧内容区 */}
          <main className="md:col-span-7 col-span-1 bg-white rounded-2xl shadow-lg border border-gray-100 p-6 flex flex-col h-full">
            {tab === 'my-articles' && (
              <div className="flex flex-col gap-4 flex-1 overflow-y-auto">
                <h2 className="text-xl font-bold mb-4 text-gray-900">自己写的文章</h2>
                {loading && <div className="text-gray-500 text-center py-10">加载中...</div>}
                  ...
                  >
                    <div className="flex flex-row h-24 relative">
                      ...
                      <div className="flex-1 flex flex-col justify-center px-5 py-3 z-20 relative">
                        <span className="text-base font-semibold text-gray-900">{article.title}</span>
                        <span className="text-sm text-gray-700 mt-1">状态：{article.status}</span>
            ...
            {tab === 'liked-articles' && (
              <div className="flex flex-col gap-4 flex-1 overflow-y-auto">
                <h2 className="text-xl font-bold mb-4 text-gray-900">点赞过的文章</h2>
                {likedArticles.length === 0 && <div className="text-gray-500 text-center py-10">暂无点赞文章</div>}
            ...
            {tab === 'change-password' && (
              <div className="flex flex-col gap-4 flex-1 overflow-y-auto">
                <h2 className="text-xl font-bold mb-4 text-gray-900">修改密码</h2>
                <ChangePasswordForm />
              </div>
            )}
          </main>
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-md space-y-4">
      <div>
        ...
        <input type="password" className="w-full border rounded px-3 py-2" value={oldPwd} onChange={e => setOldPwd(e.target.value)} />
      </div>
      <div>
      ...
      {msg && <div className={msg === "修改成功" ? "text-green-600" : "text-red-500"}>{msg}</div>}
      <button type="submit" className="bg-indigo-600 text-white px-4 py-2 rounded" disabled={loading}>{loading ? "提交中..." : "修改密码"}</button>
    </form>
  );
}
